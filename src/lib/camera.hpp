...
// The SDK returns shared_ptr, so we must hold onto it.
...
inline std::vector<handle> init() {
    ...
    std::println("Scanning for {} UNIQUE cameras (Timeout: {}s)...", target_unique_count, timeout_seconds);
    ...
    while (std::chrono::steady_clock::now() - start_time < std::chrono::seconds(timeout_seconds)) {
        if (CameraLibrary::CameraManager::X().AreCamerasInitialized()) {
            ...
            for(int i = 0; i < list.Count(); ++i) {
                auto cam_ptr = CameraLibrary::CameraManager::X().GetCamera(list[i].UID());
                if (cam_ptr) {
            ...
            if (temp_serials.size() >= target_unique_count) {
                std::println("Discovery Success: Found {} unique cameras.", temp_serials.size());
                ready = true;
                break;
            }
        }
    ...
    if (!ready) {
        std::println(stderr, "TIMEOUT: Did not find {} unique cameras in time.", target_unique_count);
    }

    // ------------------------------------------------------------
    // FINAL LIST BUILDING (Now using shared_ptr)
    // ------------------------------------------------------------
    ...
    for (int i = 0; i < list.Count(); ++i) {
        auto cam_shared = CameraLibrary::CameraManager::X().GetCamera(list[i].UID());
        ...
        if (cam_shared) {
            ...
            // STORE THE SHARED_PTR, not the raw pointer
    ...
    std::sort(handles.begin(), handles.end(), [](handle a, handle b) {
...
inline void get_frame(handle h, u8 *buffer, u32 size, u64 *out_frame_id = nullptr) {
#ifdef LIB_CAMERA_IMPLEMENTATION
    if (!h) return;
    (void)size; 
...
inline void set_ir_filter(handle h, bool enable_visible_light) {
    ...
    if (h) {
        ...
        std::println("Cam {} Filter: {}", h->Serial(), enable_visible_light ? "Visible" : "IR");
    }
#endif
}
